...
= resolve an asset_id into the exact approved Project configuration
   needed by one shot or Generation Package
```
...
6. keeps Temporary State in the shot or sequence configuration and never writes it into the Blueprint;
...
The minimum execution view may contain asset identity, type, resolved version and variant, approved Permanent Asset Locks, shot-specific Temporary State and only explicitly selected approved references. It excludes full Blueprints, private rationale, internal history, unused references and unrelated Registry entries.
...
New Shot Blueprints select assets semantically:
...
Temporary State is part of this Project/shot configuration. It never changes the Registry or Blueprint.
